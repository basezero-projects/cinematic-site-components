...
A library of 44 cinematic website modules. Each is a standalone single-file HTML demo. No build step, no frameworks.

## What's Here

- `*.html` — 44 standalone cinematic module demos (just open in browser)
...
- **Scroll-Driven** (19) — Text mask, sticky stack, zoom parallax, horizontal scroll, sticky cards, SVG draw, curtain reveal, split screen, color shift, service area map, SVG mask chapters, scroll filter distortion, WebGL rotating gallery, WebGL material reveal, curved path motion, 3D scroll text, 3D camera tour, folding package scroll, projection mapping grid
...
- **Click & Tap** (9) — View transitions, particle button, odometer, coverflow, dynamic island, dock nav, spatial product hotspots, persistent 3D transition, ink bleed overlay
